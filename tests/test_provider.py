...
assert (
    cert_pubkey.as_der() == pubkey.as_der()
), f"Public key DER comparison failed : {cert_pubkey.as_der()} != {pubkey.as_der()}"
print(f"Public key DER : {cert_pubkey.as_der().hex()}\n")

assert (
    cert_pubkey.get_modulus() == pubkey.get_modulus()
), f"Modulus comparison failed : {cert_pubkey.get_modulus()} != {pubkey.get_modulus()}"
...
print(
    f"Verification with public key : {'OK' if verified == 1 else 'NOK'} (verified={verified})\n"
)
